...
optional surfaces (Confirm UI page, image API path, house font).
...
CORE_MODULES = (
    ...
    ("yaml", "PyYAML", "register_template.py and template workspaces"),
)
...
def check_fonts() -> list[str]:
    ...
    return [
        ...
        f"python3 skills/ppt-master/scripts/install_fonts.py (repository AGENTS.md font policy)"
    ]
...
def check_image_backend() -> list[str]:
    ...
    return [
        ...
        "sourcing, or user-supplied images"
    ]


def check_codex_stubs() -> list[str]:
    """Dual-host drift gate: .codex/skills stubs must match a fresh build from
    the canonical skills/ tree. Stale stubs mean Codex triggers on outdated
    skill frontmatter."""
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    warnings = check_optional_deps() + check_fonts()
