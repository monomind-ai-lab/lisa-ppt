...
> **🔒 House-font lock — Pretendard (repository policy, declared once in the root `AGENTS.md`).** Typography is **fixed to the Pretendard family** for every SVG-authored deck: present one Pretendard plan (weight roles + size ramp) at the confirmation stage, repeated in every direction with `typography.fixed: true` — do NOT propose alternative families, and the concord/contrast pairing requirement below does not apply. Deviate only when the user explicitly names another font in the current conversation, or a selected template declares its own stacks (template precedence); everything below in this section then serves as background.
> - **Family names**: `Pretendard` with `font-weight` 400 / 700; the intermediate cuts are separate family names authored at normal weight — `"Pretendard Light"`, `"Pretendard Medium"`, `"Pretendard SemiBold"`, `"Pretendard ExtraBold"`. Exactly these six weights (300 / 400 / 500 / 600 / 700 / 800) are bundled at [`../assets/fonts/Pretendard/`](../assets/fonts/Pretendard/) (SIL OFL); never name a weight that is not bundled.
...
> - **Export**: the converter writes Pretendard into both the Latin and East-Asian typeface slots (`DUAL_SCRIPT_FONTS`); the CSS tail after it is preview-only. PPTX does not embed fonts: install the family on every authoring and viewing machine with `python3 scripts/install_fonts.py`, and record "requires Pretendard" in the Design Spec when the deck will be shared.
...
**Reference — PPT-safe faces (recall, not a whitelist; name one concrete face per script, never a comma stack)**: house family `Pretendard` and its bundled cuts (Korean + Latin, see the lock above), CJK sans `Microsoft YaHei` / `SimHei`, CJK serif `SimSun` / `FangSong` / `KaiTi` (their macOS counterparts `PingFang SC` / `Heiti SC` / `Songti SC` are preview aliases, never the named face), Latin sans `Arial` / `Calibri` / `Segoe UI` / `Verdana` / `Trebuchet MS`, Latin serif `Times New Roman` / `Georgia` / `Cambria` / `Palatino` / `Garamond`, mono `Consolas` / `Courier New`, display `Impact` / `Arial Black`. Let the locked style's character pick the axis and lead the title — `Microsoft YaHei` / `Arial` are the neutral members, never the automatic lead; a neutral sans title where the style asks for character is the failure to avoid. Non-pre-installed directions — retro/pixel Press Start 2P / VT323, rounded Nunito / Quicksand / OPPO Sans (safe substitute `Trebuchet MS` / `Verdana`), modern web Inter / HarmonyOS Sans / Source Han, calligraphic 隶书 / 华文行楷 / 华文新魏 (safe substitute `KaiTi` / `FangSong`, titles only), brand faces — need target installation or stay Design Spec references.
