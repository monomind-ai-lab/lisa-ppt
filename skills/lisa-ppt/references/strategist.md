...
> **🔒 House-font lock — one family per language (repository policy, declared once in the root `AGENTS.md`).** Typography is **fixed to the house family of the deck's language** for every SVG-authored deck — **Pretendard** for Korean, **Noto Sans TC** for Traditional Chinese, **Plus Jakarta Sans** for English, with **JetBrains Mono** for ids and code in any language: present one plan for that family (weight roles + size ramp) at the confirmation stage, repeated in every direction with `typography.fixed: true` — do NOT propose alternative families, and the concord/contrast pairing requirement below does not apply. The other house families are companions for a foreign word or an id, never a second voice. Deviate only when the user explicitly names another font in the current conversation, or a selected template declares its own stacks (template precedence); everything below in this section then serves as background.
> - **Family names**: the plain family name with `font-weight` 400 / 700; every other cut is its own family name authored at normal weight — `"Pretendard Light"` / `"Pretendard Medium"` / `"Pretendard SemiBold"` / `"Pretendard ExtraBold"`; `"Noto Sans TC Light"` / `"Noto Sans TC Medium"`; `"Plus Jakarta Sans Light"` / `"Plus Jakarta Sans Medium"` / `"Plus Jakarta Sans SemiBold"` / `"Plus Jakarta Sans ExtraBold"`; `"JetBrains Mono Medium"` / `"JetBrains Mono SemiBold"`. Exactly these weights are bundled at [`../assets/fonts/`](../assets/fonts/) (SIL OFL) — Pretendard and Plus Jakarta Sans 300 / 400 / 500 / 600 / 700 / 800, Noto Sans TC 300 / 400 / 500 / 700, JetBrains Mono 400 / 500 / 600; never name a weight that is not bundled.
> - **Style character (§2 GATE)** is realized through weight span (e.g. ExtraBold title / Light body), size ramp, letter-spacing, casing, and colour — hierarchy through weight, size, tracking and colour, never by switching families.
> - **Export**: the converter writes Pretendard and Noto Sans TC into both the Latin and East-Asian typeface slots (`DUAL_SCRIPT_FONTS`); Plus Jakarta Sans and JetBrains Mono fill the Latin slot, and an English deck that needs a CJK companion names it after the Latin face. The CSS tail after the house family — `Pretendard, "Apple SD Gothic Neo", "Malgun Gothic", "Noto Sans KR", sans-serif`; `"Noto Sans TC", "PingFang TC", "Microsoft JhengHei", "Noto Sans CJK TC", sans-serif`; `"Plus Jakarta Sans", "Segoe UI", "Helvetica Neue", Arial, sans-serif`; `"JetBrains Mono", Menlo, Consolas, monospace` — is preview-only. PPTX does not embed fonts: install the families on every authoring and viewing machine with `python3 scripts/install_fonts.py`, and record "requires <family>" in the Design Spec when the deck will be shared.
...
**Reference — PPT-safe faces (recall, not a whitelist; name one concrete face per script, never a comma stack)**: house families `Pretendard` (Korean + Latin), `Noto Sans TC` (Traditional Chinese + Latin), `Plus Jakarta Sans` (Latin) and `JetBrains Mono` (mono) with their bundled cuts (see the lock above), CJK sans `Microsoft YaHei` / `SimHei`, CJK serif `SimSun` / `FangSong` / `KaiTi` (their macOS counterparts `PingFang SC` / `Heiti SC` / `Songti SC` are preview aliases, never the named face), Latin sans `Arial` / `Calibri` / `Segoe UI` / `Verdana` / `Trebuchet MS`, Latin serif `Times New Roman` / `Georgia` / `Cambria` / `Palatino` / `Garamond`, mono `Consolas` / `Courier New`, display `Impact` / `Arial Black`. Let the locked style's character pick the axis and lead the title — `Microsoft YaHei` / `Arial` are the neutral members, never the automatic lead; a neutral sans title where the style asks for character is the failure to avoid. Non-pre-installed directions — retro/pixel Press Start 2P / VT323, rounded Nunito / Quicksand / OPPO Sans (safe substitute `Trebuchet MS` / `Verdana`), modern web Inter / HarmonyOS Sans / Source Han, calligraphic 隶书 / 华文行楷 / 华文新魏 (safe substitute `KaiTi` / `FangSong`, titles only), brand faces — need target installation or stay Design Spec references.
...
```bash
python3 skills/lisa-ppt/scripts/visualization_recall.py validate \
