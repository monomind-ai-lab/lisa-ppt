...
|---|---|---|
| `api`, or `auto` with `IMAGE_BACKEND` configured or the Codex CLI installed (the `codex` default) | **Path A** `image_gen.py --manifest` | One command runs the manifest with concurrency and writes status per item |
