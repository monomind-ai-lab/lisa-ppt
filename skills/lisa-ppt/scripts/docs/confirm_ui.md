...
- The confirm service keeps its own project-local lock at `.confirm_ui.lock`. Step 4 shuts it down before ending; concurrent projects may use different ports.
...
- `--shutdown` stops a confirm server left running for this project and exits — **idempotent** (a no-op when nothing is running). Tries a graceful `/api/shutdown`, falls back to killing the recorded pid, then clears the lock. Generate Step 4 runs this on every path so the selected port is released once the page is done.
...
The front-end loads `/api/catalogs` (served by the confirm server) and falls back to the static `/static/catalogs.json` if that route is unavailable. `/api/catalogs` returns the static file **with the `canvas` list synced live from `config.py CANVAS_FORMATS`** — the set of formats and their `dim` come from config (single source of truth, zero drift), while five-language labels / use text stay in catalogs.json (a plain fallback label is synthesized for any new id config adds). Keys: `canvas`, `modes`, `visual_styles` (grouped), `icons`, `image_usage`, `image_ai_path`, `generation_mode`, `design_spec_depth`, `delivery_purpose`. `simple-icons` is content-driven and has no option. Each catalog entry is `{ "id", "label", "label_zh", "label_zh_tw", "label_en", "label_ja", "label_ko", ... }`; descriptions use `desc_zh` / `desc_zh_tw` / `desc_en` / `desc_ja` / `desc_ko`, and `visual_styles` groups use `group_zh` / `group_zh_tw` / `group_en` / `group_ja` / `group_ko`. The front-end falls back to legacy `label` / `desc` / `group`, so old catalogs still load, but new user-facing catalog text must cover all five languages (en / ko / zh-TW / zh / ja). English labels should mirror canonical reference names (`pyramid`, `swiss-minimal`, `Path A`, `continuous`, etc.); Korean, Traditional Chinese, Simplified Chinese, and Japanese labels should be translated for users. Descriptions render inline after the option title, not as a separate selected-option line. `visual_styles` is `[{ "group", "group_zh", "group_zh_tw", "group_en", "group_ja", "group_ko", "items": [...] }]`. For `canvas` you only need to maintain the five-language labels in catalogs.json; the format set and dimensions are authoritative in `config.py CANVAS_FORMATS`.
...
- **Combined style preview** — a compact live "overall impression" strip sits just above the color section and is **sticky**: it pins under the topbar so it stays visible while the user scrolls through the color / icon / typography sections, keeping the picking controls and their combined effect on screen together. Those three sections therefore live inside the strip's own sticky container — a sticky box only travels as far as its containing block reaches. On a narrow page (≤ 700px wide) or a short one (≤ 760px tall) it stops pinning and scrolls with the chapter instead, because there the pinned strip would crowd out the very pickers it explains. It applies the currently selected color palette **and** typography (heading sample in `primary` over `background`, body sample in `body_text`, an `accent` bar, a `secondary_bg` chip) and repaints on every color / HEX-override / font / `body_size` change. It does not replace the per-candidate swatches or font samples (those stay for picking); it is deliberately an abstract style chip, **not** a slide-layout preview — the pages themselves are reviewed in PowerPoint after export. No schema field; it derives entirely from the existing color + typography selections.
...
- `lang` is the soft UI-language default (`en` / `ko` / `zh-TW` / `zh` / `ja`); the persisted user choice wins. It never sets `primary_language`.
...
- No SVG / layout preview here — the deck itself is reviewed in PowerPoint after export, or page by page with `visual_review.py` under the opt-in [`visual-review`](../../workflows/stages/visual-review.md) stage.
