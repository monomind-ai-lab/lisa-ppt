...
원본 문서 → 프로젝트 생성 → [템플릿] → 전략 확인(브라우저) → [이미지 생성]
        → SVG 페이지 작성 → 품질 검사(겹침·넘침·줄바꿈 자동 검출) → 후처리 → PPTX 내보내기
```
...
| 생성 중 | 페이지가 순서대로 작성됩니다. 고칠 부분은 채팅으로 말하면 AI가 그 페이지를 수정 |
...
**A.** 안전망은 세 겹입니다 — ① 생성 직후 **자동 지오메트리 게이트**: 텍스트 겹침·캔버스 이탈·어색한 줄바꿈을 글리프 폭 산수로 검출해 AI가 스스로 수정합니다(플래그된 페이지는 픽셀 렌더로 한 번 더 육안 확인), ② 명시 요청 시 페이지별 AI 시각 자체검토(`visual-review`), ③ 수출 후 `pptx_delivery_check.py`가 파일을 다시 읽어 패키지 무결성과 이식성을 점검. 그리고 생성 중에는 채팅으로 언제든 고쳐 달라고 할 수 있고, 최종 결과물이 편집 가능한 PPTX라서 나중에도 직접 고칠 수 있습니다.
...
`main` 기준 실측, 2026-09-03:

| | |
|---|---|
| `git clone` | **210 MB** — 작업 디렉토리 94 MB + 히스토리 116 MB |
| `git clone --depth 1` | **131 MB** — 같은 작업 디렉토리, 히스토리는 37 MB |
| 작업 디렉토리만 | 파일 12,869개, 내용 55 MB, 디스크에서는 약 94 MB |

94 MB 중 80 MB는 두 디렉토리에서 나오고, 둘 다 의도한 것입니다.

- **`skills/lisa-ppt/templates/icons/`** — SVG 12,027개. 내용은 13 MB뿐이지만
  디스크에서는 약 48 MB입니다. 파일 하나가 1 KB도 안 되는데 파일마다 파일시스템
  블록을 하나씩 차지하기 때문입니다. 그래도 파일로 둡니다. 아이콘을 받으려고
  멈추는 덱은, 결국 멈추는 덱이니까요.
- **`skills/lisa-ppt/assets/fonts/`** — 32 MB, 네 가족 19개 웨이트. PPTX는 폰트를
  임베드하지 않으므로 페이지가 지정한 가족이 그 파일을 여는 기기에 실제로 있어야
  합니다. `install_fonts.py`가 그 일을 합니다.

히스토리가 작업 디렉토리보다 무거운 이유는 v6.1.0 임포트가 업스트림의
`docs/assets/`(33 MB, 히어로 GIF)와 AI 이미지 비교 갤러리(43 MB의 PNG)를 함께
들여왔고, 둘 다 같은 커밋 묶음에서 제거됐기 때문입니다. 트리에는 없지만
히스토리에는 남아 있고, 지금은 각각 어디로 갔는지 알려주는 README 한 장만
있습니다. `--depth 1`은 이를 건너뛰며, 파이프라인은 이것들을 필요로 하지 않습니다.
...
| | 문서 | 내용 |
|---|---|---|
| 📘 | [Getting Started](docs/getting-started.md) | 첫 덱 만들기 3단계, 템플릿·애니메이션·나레이션 사용법 |
