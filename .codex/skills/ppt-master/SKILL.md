...
The authoritative skill lives in this repository at `skills/ppt-master/` — a single
...
1. Read `skills/ppt-master/SKILL.md` (repo-root relative) in full and execute it
   exactly as a runtime procedure.
2. Resolve paths: treat `${SKILL_DIR}` as `skills/ppt-master`. Bare relative paths
