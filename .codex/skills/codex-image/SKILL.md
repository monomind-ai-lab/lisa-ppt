...
The authoritative skill lives in this repository at `skills/codex-image/` — a single
...
1. Read `skills/codex-image/SKILL.md` (repo-root relative) in full and execute it
   exactly as a runtime procedure.
2. Resolve paths: treat `${SKILL_DIR}` as `skills/codex-image`. Bare relative paths
