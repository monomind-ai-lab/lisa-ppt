...
- **Bundled family: Pretendard** (SIL OFL 1.1) at [`skills/lisa-ppt/assets/fonts/Pretendard/`](skills/lisa-ppt/assets/fonts/Pretendard/) beside its licence — exactly six weights: Light 300, Regular 400, Medium 500, SemiBold 600, Bold 700, ExtraBold 800. Prompts, specs and locks may name only these; no other cut of the family exists in this repository.
- **Author it as** `Pretendard` with `font-weight` 400 / 700, and the intermediate cuts as their own family names at normal weight — `"Pretendard Light"`, `"Pretendard Medium"`, `"Pretendard SemiBold"`, `"Pretendard ExtraBold"`. The converter writes Pretendard into both the Latin and East-Asian typeface slots (`DUAL_SCRIPT_FONTS` in [`scripts/svg_to_pptx/drawingml/utils.py`](skills/lisa-ppt/scripts/svg_to_pptx/drawingml/utils.py)), so mixed Korean/Latin runs stay in one family.
...
- **Install it wherever decks are authored or opened**: `python3 skills/lisa-ppt/scripts/install_fonts.py` (user-level on macOS, Windows and Linux; `--dry-run` shows the plan, `--check` reports). PPTX does not embed fonts; a deck shared to a machine without Pretendard falls back to a system face — record "requires Pretendard" in the Design Spec when a deck will travel.
...
python3 skills/lisa-ppt/scripts/svg_editor/server.py <project_path> --live --daemon
